    ...
    for encoding in encodings:
        ...
        if True in matches:
            ...
            if name in employee:
                ...
                formatted = curtime.strftime(
                    '%d-%m-%Y')  # convert your date in a string format before inserting it to your database

                # query1 = "INSERT INTO emp_details(empname) VALUES(name)";

                query1 = "INSERT INTO attendenceregister  (Date,EmployeeID,GeoTagLocationID) VALUES ('%s','%s','%s')" % (
                formatted, emplid, geolocation);
                con.execute(query1)
                mydb.commit()
